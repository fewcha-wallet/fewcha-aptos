import { AptosAccount, MaybeHexString, Types } from "@fewcha/aptos";
...
export interface Web3ProviderStandard {
  ...
  generateBCSTransaction(accountFrom: AptosAccount, rawTxn: RawTransaction): Promise<Uint8Array>;
  generateTransaction(
  ...
  getTransactions(query?: { start?: number; limit?: number }): Promise<Types.OnChainTransaction[]>;
  ...
  signTransaction(txnRequest: Types.UserTransactionRequest): Promise<Types.SubmitTransactionRequest>;
  signAndSubmitTransaction(txnRequest: Types.UserTransactionRequest): Promise<string>;
